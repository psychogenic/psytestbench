...
class Sweep(PropertyWrapper):
    
    def __init__(self, rawProperty:scpi.scpi_instrument.Property):
        super().__init__(rawProperty)
        
    def frequencyStart(self, freqHz:float=None):
        return self.getSetFloat(self.prop.frequency.start, freqHz)
    
    def frequencyStop(self, freqHz:float=None):
        return self.getSetFloat(self.prop.frequency.stop, freqHz)
    
    def time(self, timeSecs:float=None):
        return self.getSetFloat(self.prop.time, timeSecs)
